...
except ModuleNotFoundError:  # Allows running the downloaded *_refactored.py files directly.
...
def choose_candidate_features(xn: np.ndarray, rank_day: int, prop_day: int):
    ...
    return candidates_rank, candidates_prop, masks, ranked, best_name
...
def main() -> None:
    ...
    cands_rank, cands_prop, masks, ranked, best_name = choose_candidate_features(xn, rank_day, prop_day)
    print(f"Best feature for exemplar ranking: {best_name}")
    ...
    exemplar_boxes, box_source, min_box_area = get_exemplar_boxes(args, xn, vis_rgb, masks, best_name)
    ...
    clustering_image = reduce_feature_cube(
        propagation_image,
        n_components=args.feature_components,
    ...
    save_outputs(
        ...
        exemplar_boxes,
        crown_boxes,
        minmax_scale(cands_rank[best_name]),
        density_map,
    )
