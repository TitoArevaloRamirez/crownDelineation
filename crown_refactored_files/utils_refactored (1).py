...
def reduce_feature_cube(
    ...
    """Reduce feature channels while preserving image-space coordinates.

    Parameters
    ----------
    image:
        Feature cube with shape ``[H, W, C]`` or a single ``[H, W]`` feature.
    n_components:
        Number of output channels. Values larger than the input channel count are
        clipped to ``C``.
    valid_mask:
        Optional mask used to estimate stable reduction bases from vegetation
        pixels. Non-valid or non-finite pixels are filled with valid-channel
        means before projection so background does not dominate the basis.
    method:
        ``"tucker"`` keeps the Tucker/HOSVD channel-reduction path from the
        original pipeline. Instead of returning the raw Tucker core, which rotates
        the spatial modes and is therefore unsafe for pixel-wise clustering, this
        implementation uses the Tucker channel factor to project each pixel onto
        a lower-dimensional feature basis. ``"pca"`` is a lightweight SVD
        alternative. ``"none"`` only normalizes channels.

    Returns
    -------
    np.ndarray
        Reduced and per-channel normalized feature cube with shape
        ``[H, W, n_components]``.
    """
    ...
    if method == "tucker":
        try:
            ...
            print(
                "Warning: tensorly is not installed; falling back to PCA feature reduction. "
                "Install tensorly to use --feature-reduction tucker."
            )
        else:
            # Tucker/HOSVD is run with full spatial ranks and reduced channel
            # rank. We then use only the channel factor for projection, because
            # the raw core lives in rotated spatial coordinates and should not be
            # used directly as an H x W image for clustering.
