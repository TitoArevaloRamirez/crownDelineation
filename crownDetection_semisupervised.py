...
def readData():
    ...
    root = "/home/usr3/Data/Talca2025/Registered/Darwin/"
...
def compute_feature_metrics(feature_img, mask, eps=1e-8):
    ...
    if feature_img.shape != mask.shape:
        raise ValueError(
            f"Shape mismatch: feature_img {feature_img.shape}, mask {mask.shape}"
        )
...
def main():
    ...
    masks = maskBrightSpots(candidates)
    results = rank_candidates_with_mask(
        candidates, masks=masks, sort_by="contrast_ratio"
    )
    print(results[0]["name"])

    bgr_0 = np.stack(
        (candidates_0["Blue"], candidates_0["Green"], candidates_0["Red"]), 2
    )
    ...
    if False:
        ...
        for step in pbar:
            ...
            pbar.set_description(
                "Adaptation step: {:<3}, loss: {}, predicted-count: {:6.1f}".format(
    ...
    data_matrix = np.concatenate(
        (density_map.reshape((520, 520, 1)), np.asanyarray(data_matrix)), axis=2
    )
    ...
    for i in range(0, n_c):
        ...
        for j in range(0, pts.shape[0]):
            feat_data = np.asarray(data_matrix[pts[j, 0], pts[j, 1], :]).reshape(
                (1, data_matrix.shape[2])
            )
            feat_data = np.concatenate(
                (np.array([pts[j, 0], pts[j, 1]]).reshape((1, 2)), feat_data), axis=1
            )
