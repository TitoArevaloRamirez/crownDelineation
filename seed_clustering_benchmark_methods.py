    ...
    *,
    ...
    if valid_mask is None:
        ...
        if valid_mask_arr.shape != (h, w):
            raise ValueError(
                f"valid_mask must have shape {(h, w)}; got {valid_mask_arr.shape}"
            )
    ...
    if label_supports is not None:
        ...
        for support in label_supports:
            ...
            if support_arr.shape != (h, w):
                raise ValueError(
                    f"Each support mask must have shape {(h, w)}; got {support_arr.shape}"
                )
...
def labels_nodes_to_image(
    problem: PreparedSeedProblem, labels_nodes0: np.ndarray
) -> np.ndarray:
    ...
    if labels_nodes0.shape[0] != problem.node_indices.shape[0]:
        raise ValueError(
            "labels_nodes0 length does not match the number of valid nodes"
        )
...
def _class_priors_from_argument(
    ...
    if priors.shape != (num_classes,):
        raise ValueError(
            f"class_priors must have shape {(num_classes,)}; got {priors.shape}"
        )
...
def cluster_gl_graph_nearest_neighbor(
    ...
    **_: Any,
    ...
    return _fit_graphlearning_model(
        problem, model, enforce_supports=enforce_supports, method_name="gl_graph_nn"
    )
...
def cluster_gl_laplace(
    ...
    **_: Any,
    ...
    gl_mod = _require_graphlearning()
    W, _D = get_graphlearning_matrices(
        problem, k=k, weight_kernel=weight_kernel, similarity=similarity
    )
    ...
    return _fit_graphlearning_model(
        problem, model, enforce_supports=enforce_supports, method_name="gl_laplace"
    )
...
def cluster_gl_wnll(
    ...
    **_: Any,
    ...
    return _fit_graphlearning_model(
        problem, model, enforce_supports=enforce_supports, method_name="gl_wnll"
    )
...
def cluster_gl_poisson(
    ...
    **_: Any,
    ...
    return _fit_graphlearning_model(
        problem, model, enforce_supports=enforce_supports, method_name="gl_poisson"
    )
...
def cluster_gl_peikonal(
    ...
    **_: Any,
    ...
    return _fit_graphlearning_model(
        problem, model, enforce_supports=enforce_supports, method_name="gl_peikonal"
    )
...
def cluster_gl_poisson_mbo(
    ...
    **_: Any,
    ...
    return _fit_graphlearning_model(
        problem, model, enforce_supports=enforce_supports, method_name="gl_poisson_mbo"
    )
...
def cluster_sklearn_label_propagation(
    ...
    **_: Any,
    ...
    labels0 = model.fit(
        problem.features, _sklearn_graph_labels(problem)
    ).transduction_.astype(np.int64)
...
def cluster_skimage_random_walker(
    ...
    **_: Any,
    ...
    if image is None:
        raise ValueError(
            "cluster_skimage_random_walker requires the original image= argument"
        )
    try:
    ...
    try:
        ...
        labels_img = random_walker(
            data, markers, beta=beta, mode=mode, multichannel=True
        )
    except Exception:
    ...
    if enforce_supports and problem.label_supports_nodes is not None:
        valid = labels0 >= 0
        invalid = valid & (
            ~problem.label_supports_nodes[
                np.arange(len(labels0)), np.maximum(labels0, 0)
            ]
        )
        labels0[invalid] = -1
...
def _support_images_from_problem(
    ...
    *,
    ...
    if neighborhood_radii is not None or neighborhood_radius is not None:
        ...
        return np.asarray(
            make_disk_supports(
                (h, w), seed_clusters, radius=radii, valid_mask=problem.valid_mask
            ),
            dtype=bool,
        )
...
def cluster_improved_random_walker(
    ...
    **_: Any,
    ...
    if image is None:
        raise ValueError(
            "cluster_improved_random_walker requires the original image= argument"
        )
    try:
        from skimage.segmentation import random_walker
    except Exception as exc:  # pragma: no cover
        raise ImportError(
            "scikit-image is required for improved_random_walker"
        ) from exc
    ...
    if problem.num_classes == 0 or len(problem.train_ind) == 0:
        ...
        labels0 = np.full(problem.node_indices.shape[0], -1, dtype=np.int64)
        return ClusteringResult(
            "improved_random_walker", labels_img, labels0, time.perf_counter() - start
        )

    data = _normalize_image_channels_for_random_walker(image)
    if data.shape[:2] != (h, w):
        raise ValueError(
            f"image shape {data.shape[:2]} does not match problem shape {(h, w)}"
        )
    ...
    if not enforce_supports:
        supports = np.broadcast_to(
            problem.valid_mask, (problem.num_classes, h, w)
        ).copy()
    ...
    if max_seeds_per_cluster is not None:
        ...
        for label_id in range(1, problem.num_classes + 1):
            ...
            if len(idx) <= max_seeds_per_cluster:
                ...
                chosen_local = np.linspace(
                    0, len(idx) - 1, int(max_seeds_per_cluster), dtype=np.int64
                )
    ...
    if enforce_supports and use_probability_constraints:
        try:
            probabilities = _run_random_walker(return_full_prob=True).astype(
            ...
            labels_crop = _run_random_walker(return_full_prob=False).astype(
                np.int32, copy=False
            )
    else:
        labels_crop = _run_random_walker(return_full_prob=False).astype(
    ...
    info = {
        ...
        if problem.label_supports_nodes is not None
            ...
            "disk_radius"
            if neighborhood_radius is not None or neighborhood_radii is not None
            else "valid_mask"
        ),
    }
    ...
    for method_name in methods:
        ...
        try:
            ...
            results[method_name] = ClusteringResult(
                ...
                labels_nodes0=np.full(
                    problem.node_indices.shape[0], -1, dtype=np.int64
                ),
                elapsed_s=elapsed,
                info={"error": repr(exc)},
            )
...
def result_summary(results: Mapping[str, ClusteringResult]) -> list[dict[str, Any]]:
    ...
    for name, result in results.items():
        ...
        unique = sorted(
            np.unique(result.labels_image[result.labels_image > 0]).tolist()
        )
        row = {
            "method": name,
...
def _make_background_image(
    ...
    *,
    ...
    if (h, w) != label_shape:
        raise ValueError(
            f"image shape {(h, w)} does not match label shape {label_shape}"
        )
    ...
    if channels == "auto":
        ...
        if any((j < 0 or j >= c) for j in idx):
            raise ValueError(
                f"channels {idx} are invalid for image with {c} channel(s)"
            )
        selected = img[..., idx]
        if selected.shape[2] == 1:
            ...
            bg = np.dstack(
                (selected[..., 0], selected[..., 1], np.mean(selected, axis=2))
            )
        else:
            bg = selected[..., :3]
...
def _extract_labels_image(
    result_or_labels: ClusteringResult | np.ndarray,
) -> np.ndarray:
    ...
    *,
    ...
    if valid_mask is not None:
        ...
        if valid_mask_arr.shape != label_shape:
            raise ValueError(
                f"valid_mask shape {valid_mask_arr.shape} does not match {label_shape}"
            )
    else:
        valid_mask_arr = None

    bg = _make_background_image(
        image, label_shape, channels=channels, normalize=normalize_image
    )
    ...
    for ax, (panel_name, result_or_labels) in zip(axes_flat, panels):
        ...
        if result_or_labels is not None:
            ...
            if boundary:
                boundaries = np.ma.masked_where(
                    ~_label_boundary_mask(labels), np.ones(label_shape)
                )
...
def demo_synthetic() -> dict[str, ClusteringResult]:
    ...
    seeds = [
        np.array([[20, 22], [21, 23], [19, 21]]),
        np.array([[42, 43], [43, 44], [41, 42]]),
    ]
    supports = make_disk_supports((h, w), seeds, radius=22, valid_mask=valid_mask)

    methods = [
        "sk_label_propagation",
        "sk_label_spreading",
        "skimage_random_walker",
        "improved_random_walker",
    ]
    if gl is not None:
