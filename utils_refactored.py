...
def reduce_feature_cube(
    ...
    h, w, c = x.shape
    n_components = int(max(1, min(int(n_components), c)))
    method = str(method).lower()

    if method == "none" or c <= n_components:
        return normalize_channels(x[..., :n_components])
    if method not in {"tucker", "pca"}:
        raise ValueError("method must be 'tucker', 'pca', or 'none'")
    ...
    if valid_mask is None:
        ...
        if mask.shape != (h, w):
            raise ValueError(f"valid_mask must have shape {(h, w)}, got {mask.shape}")

    flat = x.reshape(-1, c)
    finite_rows = np.all(np.isfinite(flat), axis=1)
    train_rows = mask.ravel() & finite_rows
    ...
    if np.any(invalid_or_background):
        filled_flat = filled_flat.copy()
        filled_flat[invalid_or_background] = mean

    if method == "tucker":
        x_filled = filled_flat.reshape(h, w, c).astype(np.float32, copy=False)
        try:
            _, factors = tucker(
                x_filled,
                rank=[h, w, n_components],
                init="svd",
                tol=1e-5,
                n_iter_max=50,
            )
            channel_factor = np.asarray(factors[2], dtype=np.float64)
            if channel_factor.shape != (c, n_components):
                raise RuntimeError(
                    f"Unexpected Tucker channel factor shape {channel_factor.shape}; "
            ...
            print(
                "Warning: Tucker feature reduction failed; falling back to PCA. "
                f"Details: {exc}"
            )

    centered_train = train - mean
    _, _, vt = np.linalg.svd(np.nan_to_num(centered_train), full_matrices=False)
    components = vt[:n_components].T
    projected = (filled_flat - mean) @ components
    return normalize_channels(projected.reshape(h, w, n_components).astype(np.float32))
...
def rgb_to_uint8(image: np.ndarray) -> np.ndarray:
    ...
    Accepts:
      - uint8 RGB
      - float RGB in [0, 1]
      - arbitrary-range numeric RGB
      - single-channel image, repeated to RGB
    """
    rgb = ensure_3d_image(image).astype(np.float32)
    ...
    max_val = float(np.max(rgb)) if rgb.size else 1.0
    ...
    if max_val <= 1.5 and min_val >= -0.1:
        ...
        for ch in range(rgb.shape[-1]):
            out[..., ch] = minmax_scale(rgb[..., ch]) * 255.0

    return out.astype(np.uint8)


def bbox_iou(box_a: Sequence[int | float], box_b: Sequence[int | float]) -> float:
    """Intersection-over-union for [y1, x1, y2, x2] boxes."""
    ay1, ax1, ay2, ax2 = [float(v) for v in box_a]
    by1, bx1, by2, bx2 = [float(v) for v in box_b]
    ...
    area_a = max(0.0, ay2 - ay1 + 1.0) * max(0.0, ax2 - ax1 + 1.0)
    area_b = max(0.0, by2 - by1 + 1.0) * max(0.0, bx2 - bx1 + 1.0)
...
def non_max_suppression_boxes(
    ...
    *,
    ...
    """Return indices kept after greedy non-max suppression."""
    if len(boxes) == 0:
        return []
    ...
    for idx in order:
        ...
        for kept_idx in keep:
            if bbox_iou(boxes[idx], boxes[kept_idx]) > iou_threshold:
                should_keep = False
                break
        ...
        if top_k is not None and len(keep) >= int(top_k):
            break
...
def save_clustered_tree_crown_patches(
    ...
    *,
    ...
    """
    Save clustered tree crown patches as a reusable exemplar library.

    Args:
        image_rgb:
            RGB visualization image with shape [H, W, 3].
            Can be uint8, float [0, 1], or arbitrary numeric range.

        labels:
            Integer label image with shape [H, W].
            0 = background.
            1..K = individual clustered crowns.

        output_dir:
            Directory where the patch library will be saved.

        min_area:
            Ignore crown labels smaller than this many pixels.

        padding:
            Extra pixels around each crown crop.

        source_image_id:
            Optional path or identifier for provenance.
    ...
    Saves:
        crown_0001_rgb.png
        crown_0001_mask.png
        crown_0001_masked.png
        crown_patch_metadata.jsonl
        crown_patch_summary.json

    Returns:
        List of metadata records.
    """
    rgb_u8 = rgb_to_uint8(image_rgb)
    lab = np.asarray(labels, dtype=np.int32)

    if lab.shape != rgb_u8.shape[:2]:
        raise ValueError(
            f"labels shape {lab.shape} does not match image shape {rgb_u8.shape[:2]}"
        )
    ...
    positive_labels = sorted(int(v) for v in np.unique(lab) if v > 0)

    for save_idx, label_id in enumerate(positive_labels, start=1):
        mask = lab == label_id
        area = int(mask.sum())
        ...
        tight_y1 = int(rows.min())
        tight_y2 = int(rows.max())
        tight_x1 = int(cols.min())
        tight_x2 = int(cols.max())

        y1 = max(0, tight_y1 - int(padding))
        y2 = min(h - 1, tight_y2 + int(padding))
        x1 = max(0, tight_x1 - int(padding))
        x2 = min(w - 1, tight_x2 + int(padding))
        ...
        stem = f"{prefix}_{save_idx:04d}"

        rgb_file = f"{stem}_rgb.png"
        mask_file = f"{stem}_mask.png"
        masked_file = f"{stem}_masked.png"

        Image.fromarray(crop_rgb).save(out_dir / rgb_file)
        Image.fromarray(crop_mask).save(out_dir / mask_file)
        Image.fromarray(crop_masked).save(out_dir / masked_file)

        record = {
            "patch_id": stem,
            "label_id": label_id,
            "bbox_y1x1y2x2": [int(y1), int(x1), int(y2), int(x2)],
            "tight_bbox_y1x1y2x2": [tight_y1, tight_x1, tight_y2, tight_x2],
            "area_pixels": area,
            "height": int(y2 - y1 + 1),
            "width": int(x2 - x1 + 1),
            "rgb_file": rgb_file,
    ...
    meta_path = out_dir / "crown_patch_metadata.jsonl"
    ...
    with (out_dir / "crown_patch_summary.json").open("w", encoding="utf-8") as fout:
...
def load_patch_records(
    ...
    *,
    ...
    """
    Load crown patch metadata from a patch library.

    The directory should have been created by save_clustered_tree_crown_patches().
    """
    root = Path(patch_library_dir)

    if not root.exists():
        raise FileNotFoundError(f"Patch library not found: {root}")

    records: List[dict] = []
    meta_path = root / "crown_patch_metadata.jsonl"
    ...
    if meta_path.exists():
        with meta_path.open("r", encoding="utf-8") as fin:
            for line in fin:
                ...
                rec = json.loads(line)
                rec["rgb_path"] = root / rec.get("rgb_file", "")
                rec["mask_path"] = root / rec.get("mask_file", "")
                records.append(rec)
            ...
            records.append(
                {
                    ...
                    "mask_file": mask_path.name if mask_path.exists() else None,
                }
            )

    records = [r for r in records if Path(r["rgb_path"]).exists()]

    if max_templates is not None:
        records = records[: int(max_templates)]

    return records


def match_gray_from_rgb(image: np.ndarray) -> np.ndarray:
    """
    Build a robust single-channel image for crown patch matching.

    It combines luminance and excess green.
    This works reasonably for RGB crown matching.
    For multispectral, pass a visualization RGB or modify this function to use
    NDVI/OSAVI/NIR as the matching channel.
    """
    rgb = rgb_to_uint8(image).astype(np.float32) / 255.0
    ...
    excess_green = minmax_scale(2.0 * g - r - b)

    gray = 0.55 * luminance + 0.45 * excess_green

    return normalize_to_uint8(gray)


def propose_exemplar_boxes_from_patch_library(
    image_rgb: np.ndarray,
    patch_library_dir: str | Path,
    *,
    top_k: int = 10,
    ...
    nms_iou: float = 0.3,
    max_templates: int | None = 40,
    max_candidates_per_template: int = 8,
    min_template_size: int = 5,
) -> Tuple[List[BBox], List[dict]]:
    """
    Generate exemplar boxes in a new image by matching saved crown patches.

    Args:
        image_rgb:
            New target image [H, W, 3].

        patch_library_dir:
            Directory created by save_clustered_tree_crown_patches().

        top_k:
            Maximum number of final boxes after non-max suppression.

        score_threshold:
            Minimum template matching score.

        scales:
            Template resize scales. Use multiple scales if crown size changes
            between images.
        ...
        max_templates:
            Maximum number of saved crown patches to use.

        max_candidates_per_template:
            Maximum raw detections per template and scale.

    Returns:
        boxes:
            List of [y1, x1, y2, x2] exemplar boxes in the new image.

        matches:
            Metadata for each match.
    """
    target_gray = match_gray_from_rgb(image_rgb)
    ...
    if not records:
        raise RuntimeError(f"No crown patch templates found in {patch_library_dir}")

    candidates: List[dict] = []

    for rec in records:
        rgb_path = Path(rec["rgb_path"])
        template_rgb = np.asarray(Image.open(rgb_path).convert("RGB"))
        template_gray_base = match_gray_from_rgb(template_rgb)

        mask_base = None
        mask_path = Path(rec.get("mask_path", ""))

        if mask_path.exists():
            mask_base = np.asarray(Image.open(mask_path).convert("L"))
        ...
        for scale in scales:
            ...
            th = max(1, int(round(template_gray_base.shape[0] * scale)))
            tw = max(1, int(round(template_gray_base.shape[1] * scale)))
            ...
            if mask_base is not None:
                ...
                # Masked matching is more stable with CCORR_NORMED.
                if int((mask > 0).sum()) >= min_template_size * min_template_size:
            ...
            try:
                ...
                # Some OpenCV builds are picky about mask + method combinations.
                result = cv2.matchTemplate(
                    target_gray,
                    template_gray,
            ...
            if len(scores) > int(max_candidates_per_template):
                keep = np.argsort(scores)[::-1][: int(max_candidates_per_template)]
                ys = ys[keep]
                xs = xs[keep]
                scores = scores[keep]
                ...
                candidates.append(
                    {
                        "box": box,
                        "score": float(score),
                        "patch_id": rec.get("patch_id", rgb_path.stem),
                        "scale": float(scale),
    ...
    matches = [candidates[i] for i in keep_indices]
    final_boxes = validate_bounding_boxes(
        [m["box"] for m in matches],
        (h, w),
        clip=True,
    )

    return final_boxes, matches


def write_patch_match_file(
    matches: Sequence[Mapping[str, object]],
    out_file: str | Path,
) -> None:
    """Write patch-template match metadata as JSON lines."""
    path = Path(out_file)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def boxes_to_seed_clusters_rc(
    ...
    *,
    seed_radius: int = 3,
    num_points_per_box: int = 12,
) -> List[np.ndarray]:
    """
    Convert exemplar boxes into small seed clusters in row/col coordinates.

    Each returned array is one crown seed cluster with shape [N, 2].
    This is useful for watershed, random walker, graph propagation, etc.
    """
    h, w = image_shape
    ...
    for box in boxes:
        ...
        for _ in range(max(0, num_points_per_box - 1)):
            angle = 2.0 * np.pi * rng.random()
            radius = seed_radius * np.sqrt(rng.random())

            rr = int(round(cy + radius * np.sin(angle)))
            cc = int(round(cx + radius * np.cos(angle)))
...
def filter_boxes_by_mean_area(
    ...
    *,
    ...
    """
    Remove bounding boxes whose areas are too small or too large.
    ...
    Args:
        ...
        area_std_factor:
            Keep boxes with area inside:
                mean_area +/- area_std_factor * std_area

            Used when area_ratio_range is None.

        area_ratio_range:
            Optional ratio interval around mean area.
            Example:
                (0.4, 2.5) keeps boxes with:
                0.4 * mean_area <= area <= 2.5 * mean_area

            This is often more robust than std filtering for crowns.

        min_keep:
            If filtering removes too many boxes, keep at least this many boxes
            closest to the mean area.

    Returns:
        filtered_boxes:
            Boxes that passed the area filter.

        info:
            Dictionary with area statistics.
    """
    clean_boxes = [[int(v) for v in box] for box in boxes]
    ...
    if area_ratio_range is not None:
        low_ratio, high_ratio = area_ratio_range
        low = float(low_ratio) * mean_area
        high = float(high_ratio) * mean_area
    else:
        low = mean_area - float(area_std_factor) * std_area
        high = mean_area + float(area_std_factor) * std_area

    low = max(0.0, low)

    keep_mask = (areas >= low) & (areas <= high)

    if keep_mask.sum() < min_keep and len(clean_boxes) >= min_keep:
        # Fallback: keep boxes whose area is closest to the mean.
        order = np.argsort(np.abs(areas - mean_area))
        keep_indices = order[: int(min_keep)]
        keep_mask[:] = False
    ...
    info = {
        ...
        "area_low_threshold": float(low),
        "area_high_threshold": float(high),
        "kept": int(len(filtered_boxes)),
        "removed": int(len(clean_boxes) - len(filtered_boxes)),
        "raw_areas": areas.tolist(),
    }

    return filtered_boxes, info


def detect_closed_boundary_boxes(
    image: np.ndarray,
    *,
    valid_mask: np.ndarray | None = None,
    ...
    plot_show: bool = True,
    plot_title: str = "Closed boundary detection",
):
    """
    Detect candidate object boxes using Sobel gradient boundaries.

    Key behavior:
      1. Builds an edge mask from Sobel gradient magnitude.
      2. Keeps connected edge-mask objects separated.
      3. Applies optional morphological closing locally per connected object.
      4. Extracts boxes from each separated object.
      5. Removes boxes that are much smaller/larger than the mean area.
    ...
    Returns:
        ...
        min_box_area:
            Minimum area among final boxes.
    ...
    gradient = build_sobel_gradient_image(
        x,
        channel_weights=channel_weights,
        gaussian_sigma=gaussian_sigma,
    )

    gradient_u8 = np.uint8(np.clip(gradient, 0.0, 1.0) * 255)
    ...
    if threshold_method == "otsu":
        ...
        if positive.size == 0:
            ...
            thr = float(np.percentile(positive, threshold_percentile))
            edge_mask = (gradient >= thr).astype(np.uint8) * 255
    ...
    # ------------------------------------------------------------------
    # Main change:
    # Label separated objects from edge_mask first.
    # Do NOT close the full image globally, because that merges crowns.
    # ------------------------------------------------------------------
    edge_bool = edge_mask > 0

    structure = ndi.generate_binary_structure(2, 2)
    component_labels, num_components = ndi.label(edge_bool, structure=structure)

    separated_object_mask = np.zeros((h, w), dtype=np.uint8)
    boxes_raw: List[List[int]] = []

    k = int(local_close_kernel_size)
    if k < 1:
        k = 1
    if k % 2 == 0:
        k += 1

    local_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))

    for component_id in range(1, num_components + 1):
        comp = component_labels == component_id
        ...
        # Add local padding so closing can complete small gaps
        # without touching other edge components.
        pad = max(k, 2)
        ...
        local = comp[yy1 : yy2 + 1, xx1 : xx2 + 1].astype(np.uint8) * 255

        if local_close_kernel_size > 1 and local_close_iterations > 0:
            local = cv2.morphologyEx(
                local,
                cv2.MORPH_CLOSE,
                local_kernel,
                iterations=int(local_close_iterations),
            )

        local_bool = local > 0

        if fill_holes:
            local_bool = ndi.binary_fill_holes(local_bool)

        if valid_mask is not None:
            local_valid = vm[yy1 : yy2 + 1, xx1 : xx2 + 1]
            local_bool &= local_valid

        local_mask = local_bool.astype(np.uint8) * 255
        ...
        for cnt in contours:
            ...
            lx, ly, bw, bh = cv2.boundingRect(cnt)

            if bw < int(min_width) or bh < int(min_height):
                continue

            global_x1 = int(xx1 + lx)
            global_y1 = int(yy1 + ly)
            global_x2 = int(global_x1 + bw - 1)
            global_y2 = int(global_y1 + bh - 1)

            if (
                global_x1 <= border_margin
                or global_y1 <= border_margin
                or global_x2 >= w - border_margin
                or global_y2 >= h - border_margin
            ):
                continue
            ...
            separated_object_mask[
                global_y1 : global_y2 + 1,
                global_x1 : global_x2 + 1,
            ] = np.maximum(
                separated_object_mask[
                    global_y1 : global_y2 + 1,
                    global_x1 : global_x2 + 1,
                ],
                local_mask[ly : ly + bh, lx : lx + bw],
            )

    # ------------------------------------------------------------------
    # Second requested change:
    # remove boxes much smaller or larger than mean average area.
    # ------------------------------------------------------------------
    if filter_by_mean_area:
        boxes, area_filter_info = filter_boxes_by_mean_area(
            boxes_raw,
            area_std_factor=area_std_factor,
            area_ratio_range=area_ratio_range,
            min_keep=min_area_filter_keep,
        )
    else:
        boxes = boxes_raw
        area_filter_info = {}
    ...
    debug = {
        ...
        "num_edge_components": int(num_components),
        "object_mask": separated_object_mask,
        "boxes_before_area_filter": boxes_raw,
        "boxes_after_area_filter": boxes,
        "area_filter_info": area_filter_info,
    }

    if plot:
        plot_closed_boundary_detection(
            image=image,
            boxes=boxes,
            debug={
                "gradient": gradient,
                "edge_mask": edge_mask,
                "closed_edges": edge_mask,
                "object_mask": separated_object_mask,
            },
            title=plot_title,
            show=plot_show,
...
def build_sobel_gradient_image(
    ...
    *,
    ...
    """
    Build a normalized Sobel gradient magnitude image.
    ...
    Args:
        ...
        channel_weights:
            ...
            If None, all channels are equally weighted.

        gaussian_sigma:
            Smoothing before Sobel filtering.
            Higher values reduce noise but may blur fine crown boundaries.

    Returns:
        gradient:
            Float32 image [H, W] in [0, 1].
    """
    x = np.asarray(image, dtype=np.float32)
    ...
    if channel_weights is None:
        weights = np.ones(c, dtype=np.float32) / max(c, 1)
    else:
    ...
    for ch in range(c):
        plane = minmax_scale(x[..., ch])

        if gaussian_sigma is not None and gaussian_sigma > 0:
            plane = ndi.gaussian_filter(plane, sigma=float(gaussian_sigma))
...
def plot_closed_boundary_detection(
    ...
    debug: dict,
    *,
    title: str = "Closed boundary detection",
    ...
    """
    Plot boundary-detection intermediate results and final boxes.

    Args:
        image:
            Original image [H, W] or [H, W, C].

        boxes:
            Detected boxes in [y1, x1, y2, x2] format.

        debug:
            Debug dictionary returned by detect_closed_boundary_boxes(...).
        ...
        box_color:
            Color used for drawing bounding boxes.
    ...
    if img.ndim == 2:
        display_img = minmax_scale(img)
        cmap = "gray"
    else:
        display_img = rgb_to_uint8(img)
        cmap = None

    gradient = debug.get("gradient")
    edge_mask = debug.get("edge_mask")
    closed_edges = debug.get("closed_edges")
    object_mask = debug.get("object_mask")
    ...
    axes[4].imshow(object_mask, cmap="gray")
    axes[4].set_title("Filled object mask")
    axes[4].axis("off")

    axes[5].imshow(display_img, cmap=cmap)
    axes[5].set_title(f"Detected boxes: {len(boxes)}")
    axes[5].axis("off")
